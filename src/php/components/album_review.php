...
function album_review($user_id, $disk_id, $edition_name): string
{
    ...
    $rating_value = $user_review ? $user_review['rating'] : 0;
    $radios = '<div class="stars-container" data-rating="' . $rating_value . '">';
    for ($i = 1; $i <= 5; $i++) {
        $checked = ($user_review && $user_review['rating'] == $i) ? 'checked' : '';
        $stars_label = $i == 1 ? '1 stella' : $i . ' stelle';
        $radios .= '<input type="radio" id="star' . $i . '" name="rating" value="' . $i . '" ' . $checked . ' required />
                    <label for="star' . $i . '" title="' . $stars_label . '">' . $i . '</label>';               
    }
    $radios .= '</div>';
    ...
    echo Template::render('static/layout/album/album_review_form.html', [
        ...
        'errors' => isset($errors) && !empty($errors) ? '<div id="add-artist-error-container" aria-live="assertive" class="error-message">Sono stati riscontrati i seguenti errori: <ul><li>' . $errors . '</li></ul></div>' : ''
    ]);
    return ob_get_clean();
}